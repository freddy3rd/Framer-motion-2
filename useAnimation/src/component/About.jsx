    ...
    <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 relative w-[100%] xl:gap-10 items-center">
        <div className="grid place-items-center relative h-full px-5 mb-10 xl:justify-end school">
                  ...
                  animate={(activeSchool === "HS") ? {opacity:1,scale:[0,1],display:"block"} : {opacity: 0}}
                  ...
                  <div className="card-body text-start">
                      ...
                      <div className="card-actions justify-end mt-4 relative">
                        <button  className='px-8 ff-barlow font-semibold ff-barlow text-base-200  text-md py-2 bg-slate-500 rounded-full'>VISIT</button>
                      </div>
                    </div>
                  </motion.div> 
                  <motion.div
                  ...
                  layouId="SHS"
                  initial={{display:'none'}}
                  animate={(activeSchool === "SHS") ? {opacity:1,scale:[0,1],display:"block"} : {opacity: 0}}
                  className="opacity-0 card card-compact text-[#171515] bg-white xl:w-96 w-[100%] max-w-[600px] shadow-2xl xl:translate-x-96 z-10">
                  <figure className='h-[200px] '><img src={SHS} alt="JFBSHS" width="100%" /></figure>
                  <div className="card-body text-start">
                      <h2 className="card-title text-2xl lg:text-center text-start ff-barlow capitalize">judge felician belmonte sr high school</h2>
                      <span className='text-[10px] ff-achivo font-semibold text-gray-700 uppercase'>Graduated As</span>
                      <span className=' text-gray-700 ff-monsterot text-[12px]'>TVL TRACK : ANIMATION</span>
                      <span className='flex gap-2 items-center text-lg ff-barlow uppercase'>
                      <FaDirections/>
                      Address: 
                      </span>
                  ...
                  layouId="COLLEGE"
                  initial={{display:'none'}}
                  animate={(activeSchool === "COLLEGE") ? {opacity:1,scale:[0,1],display:"block"} : {opacity: 0}}
                  className="opacity-0 card card-compact text-[#171515] bg-white xl:w-96 w-[100%] max-w-[600px] shadow-2xl xl:translate-x-96 z-10">
                  <figure className='h-[200px] '><img src={QCU} alt="JFBSHS" width="100%" /></figure>
                  <div className="card-body text-start">
                      <h2 className="card-title text-2xl lg:text-center text-start ff-barlow capitalize">quezon city university</h2>
                      <span className='text-[10px] ff-achivo font-semibold text-gray-700 uppercase'>Graduated As</span>
                      <span className=' text-gray-700 ff-monsterot'>Bachelor of Science in Information Technology</span>
                      <span className='flex gap-2 items-center text-lg ff-barlow uppercase'>
                      <FaDirections/>
                      Address: 
                      </span>
                      <li className='ff-monsterot text-xs ff-monsterot px-2'>673 Quirino Hwy, Novaliches, Quezon City, Metro Manila</li>
                      <div className="card-actions justify-end mt-4">
                        <button  className=' px-8 ff-barlow font-semibold ff-barlow text-base-200  text-md py-2 bg-slate-500 rounded-full'>VISIT</button>
                      </div>
                    </div>
        ...
        <div className="relative w-[100%] flex lg:items-center  ">
            <ul className="steps w-[100%] overflow-hidden xl:steps-vertical steps-horizontal xl:h-screen">
                ...
                data-content="✓" 
              className={`step ff-barlow text-black font-semibold hover:cursor-pointer z-20 ${(activeSchool === "SHS" || activeSchool === "COLLEGE") ? "active" : ""}`}>
                  <div className='text-start relative  px-3'>
                    <p className='xl:text-2xl text-md text-center '>JUDGE FELICIANO BELMONTE HIGH SCHOOL</p>
                ...
                className={`step ff-barlow text-black font-semibold hover:cursor-pointer z-10 ${(activeSchool === "COLLEGE") ? "active" : ""}`}>
